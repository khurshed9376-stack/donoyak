# -*- coding: utf-8 -*-
"""
Донояк TJ — веб-приложение для тестов и экзаменов
Python 3.12 + Flask
"""
...
INSTAGRAM_URL = "https://www.instagram.com/donoyak_tj/"

# Пакеты Pro (без VPS — оплата вручную, админ одобряет)
PRO_PACKAGES = {
    "hints10": {"days": 0, "price": 20, "hints": 10, "label": {"ru": "10 подсказок — 20 сомони", "en": "10 hints — 20 somoni", "tg": "10 ишора — 20 сомонӣ"}},
...
def count_exam_attempts(user_id, test_id=None):
    """Сколько раз пользователь сдавал экзамен (всего). Один раз навсегда."""
    with get_db() as conn:
        if test_id:
            row = conn.execute(
...
def has_taken_exam(user_id, test_id):
    """Экзамен можно сдавать только один раз."""
    return count_exam_attempts(user_id, test_id) >= 1


# Оценки: A 80–100, B 60–79, C 40–59, D 0–39 (F и Fx удалены)
GRADE_BANDS = [
    # letter, min, max, representative score for admin edit
    ("A", 80, 100, 90),
    ("B", 60, 79, 70),
    ("C", 40, 59, 50),
    ("D", 0, 39, 20),
...
VAPID_CLAIM_EMAIL = os.environ.get("VAPID_CLAIM_EMAIL", "mailto:admin@donoyak.tj")
...
if (
    ...
    DB_PATH = os.path.join(_data_dir, "donoyak.db")
else:
    DB_PATH = os.path.join(os.path.dirname(__file__), "data", "donoyak.db")
...
class _DBConn:
    ...
    def _adapt_sql(self, sql):
        ...
        s = s.replace("?", "%s")
        # SQLite INSERT OR REPLACE -> Postgres upsert where possible
    ...
    def execute(self, sql, params=None):
        ...
        if self._kind == "pg":
            ...
            # RETURNING id только если в SQL явно есть колонка id / типичные таблицы с serial id
            want_returning = False
            if su.startswith("INSERT") and "RETURNING" not in su:
                # таблицы без колонки id: app_settings, user_presence и т.п.
                no_id_tables = (
                    "APP_SETTINGS", "USER_PRESENCE", "PUSH_SUBSCRIPTIONS"
                )
                if not any(f"INTO {n}" in su.replace("  ", " ") or f"INTO {n} " in su for n in no_id_tables):
                    # эвристика: INSERT INTO xxx ( ... id ... или обычные таблицы users/results
                    if any(x in su for x in (
                        "INTO USERS", "INTO TEST_RESULTS", "INTO NOTIFICATIONS",
                        "INTO CONTENT_TESTS", "INTO CONTENT_QUESTIONS", "INTO PRO_REQUESTS",
                        "INTO FACULTIES", "INTO CHAT_MESSAGES", "INTO GLOBAL_NOTIFICATIONS",
                        "INTO SOCIAL_LINKS", "INTO WEBAUTHN_CREDENTIALS",
                    )):
                        want_returning = True
            sql_run = sql2
            if want_returning:
                sql_run = sql2.rstrip().rstrip(";") + " RETURNING id"
            try:
                cur.execute(sql_run, params)
            except Exception as e1:
                # транзакция могла прерваться — откат и повтор без RETURNING
                try:
                    self._raw.rollback()
                except Exception:
                    pass
            ...
            if su.startswith("INSERT"):
                try:
                    if "RETURNING" in sql_run.upper():
                        row = cur.fetchone()
                        if row is not None:
                            lastrowid = row[0] if not isinstance(row, dict) else (row.get("id") if hasattr(row, "get") else row[0])
...
def get_db():
    ...
    if DATABASE_URL:
        ...
        if _is_cloud_host():
            raise RuntimeError("PostgreSQL connection failed: %s. Check DATABASE_URL variable on Railway." % last_err)
...
def init_db():
    ...
    with get_db() as conn:
        ...
        """)
        try:
            c.execute("ALTER TABLE test_results ADD COLUMN grade TEXT DEFAULT ''")
        except Exception:
            pass
        try:
        ...
        # Гарантируем админа: admin@donoyak.tj / admin123 (и старый admin@hgu.tj)
        admin_hash = generate_password_hash("admin123", method="pbkdf2:sha256")
        for admin_email in ("admin@donoyak.tj", "admin@hgu.tj"):
            try:
                row = c.execute("SELECT id FROM users WHERE email = ?", (admin_email,)).fetchone()
                if row:
                    # всегда сбрасываем пароль и флаг is_admin — чтобы вход работал после деплоя
                    try:
                        c.execute(
                            "UPDATE users SET password_hash = ?, is_admin = 1, full_name = COALESCE(NULLIF(full_name,''), ?) WHERE email = ?",
                            (admin_hash, "Администратор", admin_email)
                        )
                    except Exception:
                        c.execute(
                    ...
                    try:
                        c.execute(
                            ...
                            ("Администратор", admin_email, admin_hash)
                        )
                    except Exception as _ae:
                        print("admin create", admin_email, _ae)
            except Exception as _ae2:
                print("admin ensure", admin_email, _ae2)
...
def error_500(e):
    ...
    try:
        ...
        print("500:", e, tb)
    # в облаке не светим traceback пользователю
    return (
        "<h2>Внутренняя ошибка</h2><p>Попробуйте обновить страницу. "
        "Если ошибка повторяется — проверьте DATABASE_URL и логи Railway.</p>",
        500,
    )
...
def error_any(e):
    # не перехватываем HTTPException (404 и т.д.)
    ...
    try:
        ...
        print("UNHANDLED:", e, tb)
    return (
        "<h2>Внутренняя ошибка</h2><p>Сервер не смог обработать запрос. "
        "Админ: проверьте логи Railway (Deployments → View Logs).</p>",
        500,
    )
...
def register():
    ...
    if request.method == "POST":
        ...
        if device_type not in ("mobile", "tablet", "desktop"):
            device_type = "mobile"
        timezone = request.form.get("timezone", "").strip() or "Asia/Dushanbe"
        ...
        try:
            ...
            with get_db() as conn:
                ...
                user_id = getattr(cur, "lastrowid", None) or getattr(conn, "lastrowid", None)
                if not user_id:
                    try:
                ...
                if not user_id:
                    try:
                        # Postgres: lastval / serial
                        row_lv = conn.execute("SELECT lastval()").fetchone()
                        user_id = row_lv[0] if row_lv else None
                    except Exception:
                        pass
                if not user_id:
                    row_tmp = conn.execute("SELECT id FROM users WHERE email = ?", (email,)).fetchone()
                    user_id = (row_tmp["id"] if hasattr(row_tmp, "keys") else row_tmp[0]) if row_tmp else None
                conn.execute(
            ...
            try:
                ...
                with get_db() as conn2:
                    conn2.execute("UPDATE users SET timezone = ? WHERE id = ?", (timezone, user_id))
                    row = conn2.execute("SELECT * FROM users WHERE id = ?", (user_id,)).fetchone()
            except Exception:
                pass
            user = User(row)
...
def api_health():
    ...
    try:
        ...
        err = str(e)[:300]
        print("health db error:", err)
    # всегда 200 — чтобы Railway healthcheck не убивал сервис из‑за краткого сбоя БД
    return jsonify({"ok": ok, "database": db_kind, "error": err or None, "app": "donoyak-tj"}), 200
        ...
        try:
            with get_db() as conn:
                ...
                if not row:
                    ...
                    ph = row["password_hash"] if hasattr(row, "keys") else row[3]
...
def start_test(test_id):
    ...
    # Только экзамен, тренировка удалена; 1 попытка навсегда
    mode = "exam"
    if not current_user.is_admin:
        if has_taken_exam(current_user.id, test_id):
            flash("Вы уже сдавали этот экзамен. Повторная сдача невозможна.", "error")
            return redirect(url_for("dashboard"))
...
def submit_test():
    ...
    if test["pro_only"] and not current_user.is_pro:
        return jsonify({"error": "Pro required"}), 403
    mode = "exam"
    if not current_user.is_admin and has_taken_exam(current_user.id, test_id):
        return jsonify({"error": "already_taken", "message": "Экзамен уже сдан. Повторная сдача невозможна."}), 403
...
def admin_test_edit(code):
    with get_db() as conn:
        ...
        if request.method == "POST":
            ...
            correct = int(request.form.get("correct") or request.form.get("correct_index") or 0)
...
def admin_result_edit(rid):
    ...
    with get_db() as conn:
        try:
            conn.execute(
                "UPDATE test_results SET score=?, max_score=?, correct=?, incorrect=?, grade=? WHERE id=?",
                (score, max_score, correct, incorrect, grade, rid)
            )
        except Exception:
            conn.execute(
...
def admin_backup():
    ...
    if backup_dir:
        try:
            ...
            dest = os.path.join(backup_dir, f"donoyak_{stamp}.db")
    ...
    if webhook:
        try:
            ...
            req.add_header("X-Donoyak-Backup", "1")
    ...
    return send_file(
        DB_PATH,
        as_attachment=True,
        download_name=f"donoyak_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db",
    )
...
def pwa_manifest():
    return jsonify({
        ...
        "description": "Донояк TJ — тесты и экзамены онлайн",
        "start_url": "/",
        "scope": "/",
        "display": "standalone",
...
self.addEventListener('push', event => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Донояк TJ', {
      body: data.body || '',
...
def api_presence():
    ...
    in_exam = 1 if data.get("in_exam") else 0
    test_id = (data.get("test_id") or "")[:64]
    now = now_tj().isoformat() if callable(globals().get("now_tj")) else datetime.now().isoformat()
    try:
        now = now_tj().isoformat()
    except Exception:
        now = datetime.now().isoformat()
    with get_db() as conn:
        conn.execute(
            """INSERT INTO user_presence (user_id, last_seen, is_in_exam, exam_test_id)
               VALUES (?, ?, ?, ?)
               ON CONFLICT(user_id) DO UPDATE SET
                 last_seen=excluded.last_seen,
                 is_in_exam=excluded.is_in_exam,
                 exam_test_id=excluded.exam_test_id""",
            (current_user.id, now, in_exam, test_id)
        )
    return jsonify({"ok": True})


def _is_online(last_seen, seconds=90):
    if not last_seen:
        return False
    try:
        ls = datetime.fromisoformat(str(last_seen).replace("Z", ""))
        if getattr(ls, "tzinfo", None):
            ls = ls.replace(tzinfo=None)
        return (datetime.utcnow() + timedelta(hours=5) - ls).total_seconds() < seconds
    except Exception:
        return False
...
def api_chat_send():
    ...
    with get_db() as conn:
        if current_user.is_admin:
            ...
            rid = int(receiver_id)
            u = conn.execute("SELECT id, is_admin FROM users WHERE id=?", (rid,)).fetchone()
        ...
        conn.execute(
            ...
            (current_user.id, rid, msg)
        )
        conn.execute(
            "INSERT INTO notifications (user_id, title, message) VALUES (?,?,?)",
            (rid, "Новое сообщение", msg[:120])
        )
        try:
            pres = conn.execute("SELECT is_in_exam FROM user_presence WHERE user_id=?", (rid,)).fetchone()
            if not (pres and pres["is_in_exam"]):
                send_push_to_user(rid, "Донояк TJ — сообщение", msg[:100], "/chat")
        except Exception:
            pass
    return jsonify({"ok": True})
...
def api_chat_messages():
    ...
    with get_db() as conn:
        ...
        return jsonify({"messages": out, "peer_id": peer_id})
...
def api_chat_peers():
    with get_db() as conn:
        users = conn.execute(
            """SELECT u.id, u.full_name, u.email, p.last_seen, p.is_in_exam
               FROM users u
               LEFT JOIN user_presence p ON p.user_id = u.id
               WHERE u.is_admin = 0
               ORDER BY p.last_seen DESC NULLS LAST, u.full_name"""
        ).fetchall()
        unread = conn.execute(
    ...
    peers = [{
        ...
        "online": _is_online(u["last_seen"]), "in_exam": bool(u["is_in_exam"]),
        "unread": unread_map.get(u["id"], 0), "last_seen": u["last_seen"],
    } for u in users]
...
@login_required
def chat_page():
    return render_template("chat.html", lang=current_user.language, is_admin=current_user.is_admin)
...
def admin_results_pdf():
    try:
        ...
        flash("Установите reportlab", "error")
    ...
    c.drawString(15 * mm, y, "Donoyak TJ — Exam Results")
    y -= 8 * mm
    c.setFont("Helvetica", 8)
    headers = ["#", "Name", "Subject", "Grade", "Score", "OK", "Bad", "Time(s)", "Date"]
    xs = [10, 25, 70, 110, 125, 145, 160, 175, 200]
    ...
    for r in rows:
        n += 1
        if y < 15 * mm:
            c.showPage(); y = h - 15 * mm; c.setFont("Helvetica", 8)
        pct = 0
        try:
            pct = round(float(r["score"]) / float(r["max_score"]) * 100, 1) if r["max_score"] else 0
        except Exception:
            pass
        try:
            grade = r["grade"] if r["grade"] else letter_grade(pct)
        except Exception:
            grade = letter_grade(pct)
        vals = [str(n), (r["full_name"] or "")[:22], (r["test_id"] or "")[:18], str(grade),
                f"{round(float(r['score'] or 0),1)}/{round(float(r['max_score'] or 0),1)}",
                str(r["correct"] or 0), str(r["incorrect"] or 0),
                str(r["duration_seconds"] or 0), str(r["created_at"] or "")[:16]]
        for i, v in enumerate(vals):
            c.drawString(xs[i] * mm, y, str(v))
        y -= 5 * mm
    c.save()
    buf.seek(0)
    return send_file(buf, as_attachment=True, download_name="donoyak_exam_results.pdf",
...
if __name__ == "__main__":
    ...
    print("Админ: admin@donoyak.tj / admin123")
